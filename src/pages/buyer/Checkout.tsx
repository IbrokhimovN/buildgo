...
import { ApiCartItem, ApiLocation, ApiOrder, ApiError, AuthError, RateLimitError } from '@/services/api';
...
const CheckoutView: React.FC<CheckoutViewProps> = ({
  cart, locations, onSuccess, onBack, submitOrder,
}) => {
  ...
  const storeGroups = cart.reduce((groups, item) => {
    ...
    return groups;
  }, {} as Record<number, { storeName: string; items: ApiCartItem[] }>);
  ...
  return (
    <div className="min-h-screen bg-surface pb-32">
      ...
      <div className="p-4 space-y-6">
        {error && (
          <div className="p-4 bg-danger-light border border-red-200 text-danger rounded-card text-sm">
            {error}
          </div>
        )}
                ...
                return (
                  <div key={item.id} className="flex justify-between text-sm py-1">
                    ...
                    <span className="font-bold">
                      {(parseFloat(price) * item.quantity).toLocaleString()} so'm
                    </span>
                  </div>
                );
              })}
